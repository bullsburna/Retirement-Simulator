...
def load_market_inputs(xlsx_path: str | Path) -> dict:
    ...
    out = {"returns_annual": pd.read_excel(wb, "Returns_Annual", index_col=0)}
    try:
...
def _portfolio_returns(returns_df: pd.DataFrame, weights: Dict[str, float]) -> pd.Series:
    ...
    if missing:
        raise ValueError(f"Tickers missing: {missing}")
...
def simulate_portfolio(
    ...
    port = _portfolio_returns(annual_rets, cfg.weights).values
    if cfg.sampling == "iid":
        draw_returns = lambda n_years: _sample_iid(port, n_years)
    elif cfg.sampling == "block":
        draw_returns = lambda n_years: _sample_block(port, n_years, cfg.block_size)
    else:
        raise ValueError("sampling must be 'iid' or 'block'")
    ...
    if draw_infl is not None:
        cum_deflator = np.ones(N, dtype=float)
    for t in range(1, Y + 1):
        contrib = float(contributions_by_year.iloc[t - 1])
        r_year = draw_returns(N)
        nominal[t, :] = (nominal[t - 1, :] + contrib) * (1.0 + r_year)
        if draw_infl is not None:
            inf = draw_infl(N)
            cum_deflator = cum_deflator / (1.0 + inf)
            real[t, :] = nominal[t, :] * cum_deflator
    def pct_df(arr: np.ndarray) -> pd.DataFrame:
        idx = pd.RangeIndex(0, Y + 1, name="Year")
        p = np.percentile(arr, [5, 25, 50, 75, 95], axis=1)
        return pd.DataFrame({"p5": p[0], "p25": p[1], "p50": p[2], "p75": p[3], "p95": p[4]}, index=idx)
    out = {
        "nominal_paths": nominal,
        "percentiles_nominal": pct_df(nominal),
        "real_paths": None,
        "percentiles_real": None,
    }
    if real is not None:
        out["real_paths"] = real
        out["percentiles_real"] = pct_df(real)
    return out
